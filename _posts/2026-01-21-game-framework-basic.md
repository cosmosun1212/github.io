...
위 코드가 왜 이런 순서로 되어 있는지, 각 함수가 유니티에서는 어떤 역할을 하는지 상세하게 뜯어보겠습니다. 아래 다이어그램은 실제 유니티 엔진의 실행 순서입니다. 우리가 만든 구조와 비교해보세요.
...
오늘 작성한 이 main.c 파일은 앞으로 우리가 만들 모든 게임의 설계도가 될 것입니다. 다음 시간에는 이 뼈대 위에 실제로 콘솔 화면을 제어하는 함수들을 추가해서 화면에 점 하나를 찍어보도록 하겠습니다.
